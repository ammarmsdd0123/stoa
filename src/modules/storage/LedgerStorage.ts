...
export class LedgerStorage extends Storages {
     ...
     */
    static readonly validator_uptime_constant = 96.42;
    ...
    public putProposals(block: Block, conn?: mysql.PoolConnection) {
        ...
        function getProposalInfo(storage: LedgerStorage, proposal_id: string, app_name: string): Promise<any[]> {
            let sql = `SELECT 
        ...
        function putPayload(storage: LedgerStorage, block_header: BlockHeader, tx_hash: Hash, tx: Transaction) {
            return new Promise<void>(async (resolve, reject) => {
                try {
                    ...
                    let header: Buffer;
                    try {
                        header = Utils.readBuffer(buffer, length);
                    }
                    catch (err) {
                        return resolve();
                    }
                    ...
                    switch (header.toString()) {
                        ...
                        case BallotData.HEADER: {
                            let buffer = SmartBuffer.fromBuffer(tx.payload);
                            let ballotData: BallotData = BallotData.deserialize(buffer);
                            let info = await getProposalInfo(storage, ballotData.proposal_id, ballotData.app_name);
                            if (info.length)
                                ((block_header.height.value >= info[0].vote_start_height) && (block_header.height.value <= info[0].vote_end_height)) ?
                                    await save_ballot_data(storage, block_header, tx_hash, ballotData) :
                                    await save_ballot_data(storage, block_header, tx_hash, ballotData, BallotData.REJECT);
                            break;
                        }
                        default: {
                            break;
    ...
    public putProposalResult(block: Block, conn?: mysql.PoolConnection) {
        ...
        function getSignCount(storage: LedgerStorage, address: string, vote_start_height: number, vote_end_height: number) {
            let sql = `SELECT 
                       count(*) as sign_count
                    FROM 
                        validator_by_block
            ...
            return storage.query(sql, [SignStatus.SIGNED, address, vote_start_height, vote_end_height]);
        }
        ...
        function getProposalVotesByValidator(storage: LedgerStorage, proposal_id: string, voter_address: string): Promise<any> {
            let sql = `SELECT 
                        proposal_id,         
                        block_height,       
                        app_name,           
                    ...
                    WHERE 
                        ...
                        voter_address = ?
                        AND 
                        sequence = (SELECT MAX(sequence) from ballots)
        `;
            return storage.query(sql, [proposal_id, voter_address]);
        }

        function updateProposals(storage: LedgerStorage, proposal_id: string, proposal_status: ProposalStatus, proposal_result: ProposalResult, conn?: mysql.PoolConnection) {
            return new Promise<void>(async (resolve, reject) => {
                storage
                    .query(`
                UPDATE 
        ...
        function validatePreImage(validator: any): IPreimage {
            ...
            if (
                ...
                if (JSBI.equal(target_height.value, JSBI.BigInt(validator.enrolled_at))) {
                    preimage_height_str = "0";
                    result_preimage_hash.fromBinary(validator.commitment, Endian.Little);
                } else {
                    preimage_height_str = "";
        ...
        function process_ballots(storage: LedgerStorage, validator: IValidatorByBlock, proposal: IProposal, votingResult: IVotingResult, conn?: mysql.PoolConnection) {
            return new Promise<void>(async (resolve, reject) => {
                try {
                    const total_block = proposal.vote_end_height - (proposal.vote_start_height - 1);
                    const sign_count = await getSignCount(storage, validator.address, proposal.vote_start_height, proposal.vote_end_height);
                    ...
                    const { height, hash } = await validatePreImage(validator_info[0]);

                    if (Number(uptime) > LedgerStorage.validator_uptime_constant && height && hash) {
                        getProposalVotesByValidator(storage, proposal.proposal_id, validator.address)
                                        ...
                                        case 0:
                                            {
                                                ...
                                                await updateValidatorBallots(storage, validator.address, ballot[0].sequence, BallotData.YES, conn);
                                                break;
                                            }
                                        case 1:
                                            {
                                                votingResult.opposed = JSBI.add(votingResult.opposed, JSBI.BigInt(1))
                                                await updateValidatorBallots(storage, validator.address, ballot[0].sequence, BallotData.NO, conn);
                                                break;
                                            }
                                        case 2:
                                            {
                                                votingResult.abstain = JSBI.add(votingResult.abstain, JSBI.BigInt(1))
                                                await updateValidatorBallots(storage, validator.address, ballot[0].sequence, BallotData.BLANK, conn);
                                                break;
                                            }
                                    }
                                }
        ...
        function computeResult(votingResult: IVotingResult, Vn: number): boolean {
            ...
            Qn = Object.values(votingResult).reduce((sum, m) => JSBI.add(sum, m), JSBI.BigInt(0))
            if (Qn > quorum) {
            ...
            (async () => {
                try {
                    ...
                    for (let proposal_index = 0; proposal_index < proposal.length; proposal_index++) {
                        ...
                        for (let validator_index = 0; validator_index < validators_by_block.length; validator_index++) {
                            await process_ballots(this, validators_by_block[validator_index], proposal[proposal_index], VotingResult, conn)
                        }
                        let result = await computeResult(VotingResult, validators_by_block.length);
                        await updateProposals(this, proposal[proposal_index].proposal_id, ProposalStatus.CLOSED, result ? ProposalResult.PASSED : ProposalResult.REJECTED, conn);
                    }
                } catch (err) {
                    reject(err);
                    return;
